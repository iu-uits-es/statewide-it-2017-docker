...
export const completeTodo = (id) => (dispatch) => {
    ...
    fetch('api/todo/' + id, { method: 'PATCH', body: JSON.stringify({ done: true }), headers: headers })
...
export const createTodo = (todo) => (dispatch) => {
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    fetch('api/todo', { method: 'POST', body: JSON.stringify(todo), headers: headers })
...
const completeTodoHandler = (state, action) => ({
    ...state,
    todos: state.todos.map(todo => todo.id === action.payload.id ? { ...todo, done: true } : todo)
});
